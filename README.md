...
界面名称 **FunnyAi对弈Of象棋 — ZZH** · 仓库 [FunnyCChessAi](https://github.com/Zhang-zu-hao/FunnyCChessAi) · 默认玩法 **揭棋**

[功能](#功能) · [玩法](#玩法) · [快速开始](#快速开始) · [AI](#ai-难度与对战) · [训练](#训练) · [许可证](#许可证)
...
[揭棋](https://image.qqchess.qq.com/test_160324/activity/html/jieqiguize.html) 等变体是不完全信息棋：暗子不能直接丢给标准象棋引擎。本项目把 **规则、搜索、Web 对局、可插拔模型** 放在同一套服务里：

- 浏览器打开即可 **人机**、**AI 对战** 或联机；大厅点选玩法会弹出规则与图示
- 中国象棋走 [Pikafish](https://github.com/official-pikafish/Pikafish)；揭棋走专用规则树（negamax / 迭代加深 / 静态搜索），高强度档可叠加皮卡鱼开局提示
- 难度 **1–10** 与 **自训练**（`engines/zzh/<mode>.pt` 或 HTTP）；未接入权重时回退该玩法最强内置搜索
...
标准象棋强度依赖 [Pikafish](https://github.com/official-pikafish/Pikafish)。请自行将二进制与网络文件放到 `engines/pikafish` 与 `engines/pikafish.nnue`（可用环境变量或符号链接）。未放置时象棋人机回退启发式，揭棋仍使用内置搜索。
...
大厅 **AI 对战**：为红、黑分别选择引擎（皮卡鱼 / 启发式 / 变体搜索 / 自训练 / 你上传的本地权重）和难度。也可在对局席位下拉框中途换引擎。

「选择本地 AI」会把 `.pt` 存到 `engines/local/`（不入库）。自训练权重体积较小，随仓库发布。
...
| `XIANGQI_ZZH_URL` | 自训练档 HTTP 模型（约定同上） |
| `XIANGQI_ZZH_DIR` | 自训练权重目录，默认 `engines/zzh` |
| `XIANGQI_LOCAL_AI_DIR` | 用户上传的本地权重，默认 `engines/local` |
...
FunnyCChessAi/
├── app/                 # FastAPI 服务与对局逻辑
│   ├── catalog.py       # 玩法、难度、界面标题
│   ├── main.py          # HTTP / WebSocket / 本地 AI 上传
│   ├── rooms.py         # 房间、人机与 AI 对战调度
│   ├── game/            # 各玩法规则
...
| `GET /api/ais?mode=` | 当前玩法可选引擎（含本地权重） |
| `POST /api/local-ai` | 上传 `.pt` 到 `engines/local/` |
| `WS /ws` | `create`（`vs=ai\|human\|aivsai`）/ `join` / `move` / `seat` / `new_game` / `resign` / `level` |

## 测试

```bash
python -m unittest tests.test_game tests.test_ai tests.test_variants tests.test_ai_match -v
```
